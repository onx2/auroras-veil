...
use crate::{screens::Screen, spacetime::SpacetimeDB, stdb::create_character};
use bevy::prelude::*;
...
fn create_character(mut next_screen: ResMut<NextState<Screen>>, stdb: SpacetimeDB) {
    println!("Screen::CharacterSelect -> create_character");
    match stdb.reducers().create_character("Jeff".into()) {
        Ok(_) => {
